(()=>{
  ...
  function brandUI(){
    ...
    const style=document.createElement('style');style.textContent=`.logo{display:flex;align-items:center;gap:9px;font-size:28px;font-weight:950;letter-spacing:-1.8px}.brand-mark{width:34px;height:34px;flex:0 0 34px;display:block}.brand-word-se{color:var(--pink)}@media(max-width:430px){.logo{gap:7px;font-size:25px}.brand-mark{width:31px;height:31px;flex-basis:31px}}`;document.head.appendChild(style);
    logo.setAttribute('aria-label','ZdrapSe');logo.innerHTML=`<svg class="brand-mark" viewBox="0 0 64 64" aria-hidden="true"><rect width="64" height="64" rx="15" fill="#17181d"/><path d="M13 14h39L29 43h24v8H11l23-29H13z" fill="#c9ff38"/><path d="M47 10l6-5M52 17l8-1M48 24l7 4" stroke="#ff5b2e" stroke-width="4" stroke-linecap="round"/></svg><span>Zdrap<span class="brand-word-se">Se</span></span>`;
  }
  ...
  function send(event){window.dataLayer=window.dataLayer||[];window.dataLayer.push({event:`zdrapse_${event.name}`,...event.props});window.dispatchEvent(new CustomEvent('zdrapse:analytics',{detail:event}));remember(event);posthog.capture(`zdrapse_${event.name}`,{...event.props,session_id:event.session_id,path:event.path,shared_id:event.shared_id,referrer:event.referrer,event_ts:event.ts})}
